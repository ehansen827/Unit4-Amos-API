...
{
    public class OrderAmos : Worker<WorkerParameters, WorkerSettings>
    {
        private readonly WorkerSettings _settings;
        private readonly ILogger<Task> _workerLogger;
        private readonly IGetHttpClient _getHttpClient;

        public OrderAmos(ILogger<Task> _workerLogger, WorkerSettings _settings, IGetHttpClient _getHttpClient)
        {
        ...
        public override  async  Task<JobResult> Execute(WorkerParameters parameters)
        {
            try
            {
                ...
                var SQLSelectPO = $@"SELECT OrderID, a.FormNo, OrderedDate, FormStatus, VendorId, uc.Name as Responsible, ua.Name as SuperInt, EstimatedTotal, i.InstCode, i.InstName, d.comment1 as Comment1, cc.CostCentreCode as Account  
                                        FROM Orderform a 
						                INNER JOIN amosuser uc ON CreatedBy = uc.UserID
						                INNER JOIN amosuser ua ON SentBy = ua.UserID
						                INNER JOIN department d ON a.DeptID = d.DeptID
										...
										LEFT JOIN a1ar_apoready apor ON a.FormNo = CAST(apor.Order_id AS varchar(99))
                                        WHERE Formtype = 1 AND Formstatus = 1 
                                        AND OrderedDate IS NOT NULL
                ...
                foreach (var value in dbConnectionAmos.Query<OrderForm>(SQLSelectPO, new { LastSuccessFulRun, _settings.DelayMins }))
                {
                    ...
                    if (installationOk == true && supplierOk == true) InsertORD(FormNo.ToString(), Vendor, Responsible, Amount, OrderDate, Account, InstName, SuperInt);
                }
                // Cancelled orders
                ...
                foreach (var value in dbConnectionAmos.Query<OrderForm>(SQLSelectCancelledPO, new { LastSuccessFulRun, _settings.DelayMins }))
                {
                    ...
                    CancelORD(value.FormNo.ToString());
                }
                // Opened orders
                ...
                foreach (var order in dbConnectionAmos.Query<OrderForm>(SQLSelectOpenedPO, new { LastSuccessFulRun, _settings.DelayMins }))
                {
                    ...
                    OpenedORD(order.FormNo.ToString());
        ...
        private JobResult InsertORD(string purchaseOrderNo, double apar_id, string responsible, double amount, DateTime orderDate, string Account, string InstName, string SuperInt)
        {
            ...
            try
            {
                dbConnectionAmos.Execute(SQLStringInsert, new { order_id = purchaseOrderNo, apar_id, responsible, amount, orderdate = orderDate, Account, InstName, SuperInt });
            }
            catch (Exception ex)
            {
                _workerLogger.LogError(ex.ToString());
        ...
        private JobResult CancelORD(string purchaseOrderNo)
        {
            ...
            try
            {
                dbConnectionAmos.Execute(SQLStringUpdate, new { order_id = purchaseOrderNo });
            }
            catch (Exception ex)
            {
                _workerLogger.LogError(ex.ToString());
